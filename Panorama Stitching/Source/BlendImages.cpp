...
{
    //printf("EnteringImageBoundingBox");
...
static void AccumulateBlend(CByteImage& img, CFloatImage& acc, CTransform3x3 M, float blendWidth)
{
    ...
    for (int y = bb_min_y; y <= bb_max_y; y++) {
        for (int x = bb_min_x; x < bb_max_x; x++) {
            ...
            //feathering piecewise "hat" function for blending
            ...
            if (x_src < blendWidth){
                ...
                weight = 1.0 - (blendWidth - (img.Shape().width - x_src))/blendWidth;
            }
...
static void NormalizeBlend(CFloatImage& acc, CByteImage& img)
{
    ...
    for(int x = 0; x < acc.Shape().width; x++){
        for(int y = 0; y < acc.Shape().height; y++){
            ...
            double intensity = acc.Pixel(x,y,3);
            
            if (intensity > 0){
                //divide by instensity to normalize pixel intensity
                for (int i=0; i<3; i++){
                    img.Pixel(x,y,i) = acc.Pixel(x,y,i)/intensity;
                }
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
    ...
    //first image height, last image height
    float firstHeight, lastHeight;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        //transform corners relative to first image
        ...
        //here, min_x is established min, corners[0][0] is min of transformed current image
        min_x = MIN(min_x, corners[0][0]);
        min_y = MIN(min_y, corners[0][1]);
        
        max_x = MAX(max_x, corners[1][0]);
        max_y = MAX(max_y, corners[2][1]);
		// *** END TODO #1 ***
        
        //code for drift, keep track of first and last image top left corner
        if(i==0){
            firstHeight = corners[2][1];
        }
        
        if(i==n-1){
            lastHeight = corners[2][1];
        }
    ...
    A[0][0] = 1;
    A[0][1] = 0;
    A[0][2] = width/2; //trim left edge
        
    A[1][0] = (lastHeight-firstHeight)/cShape.width; //takes care of vertical difting
    A[1][1] = 1;
    A[1][2] = 0;
